...
int DNNNodeSample::SetNodePara() {
  ...
  dnn_node_para_ptr_->model_file =
      "./config/yolo8_pose_bayese_640x640_nv12_modified.bin";
  // 指定算法推理任务类型
...
int DNNNodeSample::PostProcess(
  ...
  std::vector<std::shared_ptr<hobot::dnn_node::dnn_node_sample::YoloV8Result>>
      results;
  ...
  for (auto& rect : results) {
    ...
    // target.points.type = "car";
    ai_msgs::msg::Point point;
    for(int i = 0; i < 12; i += 3){
      geometry_msgs::msg::Point32 pt;
      pt.x = rect->kpts[i];
      pt.y = rect->kpts[i + 1];
      point.point.push_back(pt);
      // point.set__confidence(rect->kpts[i + 2]);
    }
  ...
  if (sample_node_output->x_ratio != 1.0 || sample_node_output->y_ratio != 1.0) {
    // 前处理有对图片进行resize，需要将坐标映射到对应的订阅图片分辨率
    for (auto& target : pub_data->targets) {
      for (auto& roi : target.rois) {
        roi.rect.x_offset *= sample_node_output->x_ratio;
        roi.rect.y_offset *= sample_node_output->y_ratio;
        roi.rect.width *= sample_node_output->x_ratio;
        roi.rect.height *= sample_node_output->y_ratio;
      }
      for(auto& points : target.points) {
        for(auto& point : points.point){
          point.x *= sample_node_output->x_ratio;
          point.y *= sample_node_output->y_ratio;
        }
        // points.point[0].x *= sample_node_output->ratio;
        // points.point[0].y *= sample_node_output->ratio;
      }
    }
  }
